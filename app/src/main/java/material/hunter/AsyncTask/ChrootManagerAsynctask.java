    ...
    protected Void doInBackground(Object... objects) {
        switch (ACTIONCODE) {
            ...
            case CHECK_CHROOT:
                ...
                break;
            case MOUNT_CHROOT:
                resultCode = exe.RunAsRootOutput(NhPaths.APP_SCRIPTS_PATH + "/bootroot", ((TextView) objects[0]));
                exe.RunAsRootOutput("sleep 1 && " + NhPaths.CHROOT_INITD_SCRIPT_PATH, ((TextView) objects[0]));
                break;
            case UNMOUNT_CHROOT:
                resultCode = exe.RunAsRootOutput(NhPaths.APP_SCRIPTS_PATH + "/killroot", ((TextView) objects[0]));
                break;
            case INSTALL_CHROOT:
                resultCode = exe.RunAsRootOutput(NhPaths.APP_SCRIPTS_PATH + "/chrootmgr -c \"restore " + objects[1].toString() + " " + objects[2].toString() + "\"", ((TextView) objects[0]));
                break;
            case REMOVE_CHROOT:
            ...
            case DOWNLOAD_CHROOT:
                try {
                    ...
                    BufferedOutputStream writer = new BufferedOutputStream(new FileOutputStream(objects[2].toString()));
