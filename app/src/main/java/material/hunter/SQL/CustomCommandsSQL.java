...
public class CustomCommandsSQL extends SQLiteOpenHelper {
  ...
  public static synchronized CustomCommandsSQL getInstance(Context context) {
    if (instance == null) {
      instance = new CustomCommandsSQL(AppNavHomeActivity.context);
    }
    return instance;
  }
  ...
  private boolean isOldDB(String storedDBpath) {
    ...
    if (ifTableExists(tempDB, oldDBTableName)) {
      Cursor c =
          tempDB.rawQuery(
              "SELECT name FROM sqlite_master WHERE type='table' AND name='" + oldDBTableName + "'",
              null);
      if (c.getCount() == 1) {
        c.close();
    ...
    try {
      ...
      currentDB.execSQL(
          "INSERT INTO "
              ...
              + ")"
              + " SELECT "
              + "ID,BTN_LABEL,COMMAND,SEND_TO_SHELL,EXEC_MODE,RUN_AT_BOOT"
